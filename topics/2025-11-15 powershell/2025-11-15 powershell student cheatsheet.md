...
<table style="font-size: 0.85em; line-height: 1.0;">
  ...
  <tr>
    ...
    <td style="padding: 2px;">PowerShell Cheat Sheet</strong></td>
  </tr>
</table>
...
* `foreach [ $var in <list>]; { ... }` → looping over a group of objects
* `command | ForEach-Object { ... }` → looping over objects received from pipeline
* `command | Where-Object { ... }` → filtering objects received from pipeline
